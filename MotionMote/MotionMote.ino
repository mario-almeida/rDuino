...
//*********************************************************************************************
#define NODEID        29   //unique for each node on same network
...
#define MOTION_SENS       // uncomment to enable reed switch
//*********************************************************************************************
#ifdef BATT_SENS
float batteryVolts;           // current battery reading
float batteryPrevVolts;       // previous battery reading
#define BATT_HISTERESIS 0.05   // Difference between current and previous reading to send to the gateway.
...
byte sendLen;
String sendData;
byte sleepInterval = 2;       // Go to sleep for sleepInterval * sleepTime = ?sec i.e 8 * 8 = 64sec
unsigned long previousSleepTime = millis();
...
void setup() {
  ...
  DEBUG(F("\nMotionMote : ")); DEBUG(FREQUENCY == RF69_433MHZ ? 433 : FREQUENCY == RF69_868MHZ ? 868 : 915); DEBUG(F(" Mhz,"));
  ...
  DEBUGln("MOTION sens pin enabled");
#endif
...
void loop() {
  ...
  //************ MOTION Reading                                                 *************
  //*********************************************************************************************
  int ms = digitalRead(MOTION_PIN);
  if (ms != motionState) {
    sendData = " MO1:" + (String)ms;
  }
